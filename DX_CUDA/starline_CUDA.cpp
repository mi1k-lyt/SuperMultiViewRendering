...
CudaResource::CudaResource() {
    ...
    delete[] h_depth_rasterize_min;
    delete[] h_depth_rasterize_max;
    ...
    void* _tex_depth_img_left = malloc(sizeof(float) * src_depth_N);
    tex_depth_img_left = reinterpret_cast<float*>(_tex_depth_img_left);

    void* _tex_depth_img_mid = malloc(sizeof(float) * src_depth_N);
    tex_depth_img_mid = reinterpret_cast<float*>(_tex_depth_img_mid);

    void* _tex_depth_img_right = malloc(sizeof(float) * src_depth_N);
    tex_depth_img_right = reinterpret_cast<float*>(_tex_depth_img_right);
...
void CudaResource::LoadImg() {
    ...
    //归一化参数:5000
    for (int i = 0; i < src_depth_N; ++i) {
        *(tex_depth_img_left + i) = (*(p_h_depth_img_left + i));
        *(tex_depth_img_mid + i) = (*(p_h_depth_img_mid + i));
        *(tex_depth_img_right + i) = (*(p_h_depth_img_right + i));
    }
}
...
void CudaResource::DestroyResource() {
    ...
    free(tex_depth_img_left);
    free(tex_depth_img_mid);
    free(tex_depth_img_right);
}
...
void ray_cast(
    ...
    ray r(u, v);
    /*      确定光线步长范围        */
    float depth_max = tex1Dfetch<float>(depth_tex_rasterize_max, point_splatting_id);//-108.059;
    float depth_min = tex1Dfetch<float>(depth_tex_rasterize_min, point_splatting_id);//-65.9798;
    ...
    Vertex* current_crood = d_crood + id;
    ...
    //    if (camera_pos_left.z >= (color_shadowmap_depth_left - 4.0f) && camera_pos_left.z <= (color_shadowmap_depth_left + 4.0f)) {
    //        current_crood->Color.x = tex2D<uchar4>(color_tex_left, int(pixel_left.x) + crop_src_pixel_color_Nx, int(pixel_left.y) + crop_src_pixel_color_Ny).z;
    //        current_crood->Color.y = tex2D<uchar4>(color_tex_left, int(pixel_left.x) + crop_src_pixel_color_Nx, int(pixel_left.y) + crop_src_pixel_color_Ny).y;
    //        current_crood->Color.z = tex2D<uchar4>(color_tex_left, int(pixel_left.x) + crop_src_pixel_color_Nx, int(pixel_left.y) + crop_src_pixel_color_Ny).x;
    //    }
    //    ////PCF 7*7计算平均可见性
    ...
    //    if (camera_pos_right.z >= (color_shadowmap_depth_right - 4.0f) && camera_pos_right.z <= (color_shadowmap_depth_right + 4.0f)) {
    //        current_crood->Color.x = tex2D<uchar4>(color_tex_right, int(pixel_right.x) + crop_src_pixel_color_Nx, int(pixel_right.y) + crop_src_pixel_color_Ny).z;
    //        current_crood->Color.y = tex2D<uchar4>(color_tex_right, int(pixel_right.x) + crop_src_pixel_color_Nx, int(pixel_right.y) + crop_src_pixel_color_Ny).y;
    //        current_crood->Color.z = tex2D<uchar4>(color_tex_right, int(pixel_right.x) + crop_src_pixel_color_Nx, int(pixel_right.y) + crop_src_pixel_color_Ny).x;
    //    }
    //    ////PCF 7*7计算平均可见性
    //    //for (int it = 0; it < 49; ++it) {
    ...
    //    if (camera_pos_mid.z >= (color_shadowmap_depth_mid - 4.0f) && camera_pos_mid.z <= (color_shadowmap_depth_mid + 4.0f)) {
    //        current_crood->Color.x = tex2D<uchar4>(color_tex_mid, int(pixel_mid.x) + crop_src_pixel_color_Nx, int(pixel_mid.y) + crop_src_pixel_color_Ny).z;
    //        current_crood->Color.y = tex2D<uchar4>(color_tex_mid, int(pixel_mid.x) + crop_src_pixel_color_Nx, int(pixel_mid.y) + crop_src_pixel_color_Ny).y;
    //        current_crood->Color.z = tex2D<uchar4>(color_tex_mid, int(pixel_mid.x) + crop_src_pixel_color_Nx, int(pixel_mid.y) + crop_src_pixel_color_Ny).x;
    //    }
    //    ////PCF 7*7计算平均可见性
    //    //for (int it = 0; it < 49; ++it) {
    ...
    //current_crood->Color.x /= 255;
    //current_crood->Color.y /= 255;
    //current_crood->Color.z /= 255;
    //current_crood->Color.w = 1.0f;



    current_crood->Pos.x = r.ray_pos.x;
    current_crood->Pos.y = -r.ray_pos.y;//坐标系变换到DX的左手坐标
    current_crood->Pos.z = r.ray_pos.z;
}